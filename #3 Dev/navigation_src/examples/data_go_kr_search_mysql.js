...
app.get('/search', function(req, res) {
  ...
  queryParams += '&' + encodeURIComponent('numOfRows') + '=' + encodeURIComponent('1000'); /* 표출건수 */
  ...
  request({
      ...
      parser.parseString(body, function(err, result) {
        ...
        for (var i = 0; i < length; ++i)
        {
          ...
          if ('A0101' <= cat2 && cat2 <= 'A0206')
          {
            ...
            var insertSQL = 'insert into tour (title, longitude, latitude, addr, img, tel) values(?, ?, ?, ?, ?, ?)';
            var insertParams = [title, longitude, latitude, addr, image, tel];
            console.log(title, longitude, latitude, addr, image, tel); // 쿼리 문을 DB에 날린 후 DB가 결과문을 반환할 때 쯤에는 이미 for문이 끝까지 돌아가서 title이 변경되있다.
            // console.log(longitude);

            // set sql param

            conn.query(insertSQL, insertParams, function(error, rows) {
              if (error) {
                console.log(error);
              }
              else {
      ...
      }); // end parser.parseString(body, function(err, result)
  }); // end request
}); // end app.get
